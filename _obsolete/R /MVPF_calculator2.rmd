...
calculate_mvpf_df <- function(
    ...
    ST_govt_rows     = c("ccj_funding_2018"),
    LT_govt_rows     = c("inc_conv_len"),
    data_sheet       = MVPF_data # Not used in code, but kept for context
) {

 #--- Detainee Value ---#
  # Assuming detainee values are a negative impact (cost/loss)
  LT_det <- sum_rows(LT_detainee_rows, sign = -1, label = "Long-term value to detainee")
  ST_det <- sum_rows(ST_detainee_rows, sign = -1, label = "Short-term value to detainee")
  
  #--- Society Value ---#
  # Assuming LT_soc is negative, ST_soc is positive
  LT_soc <- sum_rows(LT_society_rows, sign = -1, label = "Long-term value to society")
  ST_soc <- sum_rows(ST_society_rows, sign = +1, label = "Short-term value to society")
  
  #--- Government Costs ---#
  # Assuming both are positive costs
  LT_gov <- sum_rows(LT_govt_rows, sign = +1, label = "Long-term cost to government")
  ST_gov <- sum_rows(ST_govt_rows, sign = +1, label = "Short-term cost to government")
  ...
  N_det     <- or_default(value_2025("n_detainees")$v, 1)
  N_society <- or_default(value_2025("n_society")$v, 1)
  N_jail    <- or_default(value_2025("n_jail_flow")$v, 1) # Retained for potential future use
  ...
  # --- Total Values (Numerator Components) ---#
  # Per-detainee/per-society totals (before weighting)
  det_per_unit <- LT_det$total + ST_det$total
  soc_per_unit <- LT_soc$total + ST_soc$total

  # Total Benefit (Numerator Components, Weighted)
  det_total <- det_per_unit * N_det 
  soc_total <- soc_per_unit * N_society
  
  # Government Total (Denominator Component)
  # Assuming Gov costs are annual total (not per-person)
  gov_total <- LT_gov$total + ST_gov$total 
  
  cat("det_total:", det_total, "soc_total:", soc_total, "gov_total:", gov_total, "\n")
  
  #--- MVPF Calculation ---#
  numerator_total   <- soc_total + det_total
  denominator_total <- gov_total
  mvpf_value <- if (denominator_total == 0) NA_real_ else numerator_total / denominator_total
  ...
  tibble(
    alternative   = alt_id,
    # Store the ST/LT components as per-unit values (no N-weighting)
    ...
    # Store the final total weighted values
    #det_total = det_total,
    #soc_total = soc_total,
    #gov_total = gov_total,
